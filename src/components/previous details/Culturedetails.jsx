...
const Culturedetails = () => {
    ...
    const  alldata =()=>{
      axios.get(`${process.env.REACT_APP_DEVELOPMENT}/api/itemsCulture`,data,
  ...
  const onSubmit = (stock) =>{
   ...
   axios.post(`${process.env.REACT_APP_DEVELOPMENT}/api/getPrevStockCulture`,{from:date.format(value,'YYYY/MM/DD'),to:date.format(value1,'YYYY/MM/DD'), ...obj},)
      .then(response=>{
...
console.log(data.pre,'data')
  ...
  return (
    <div>
        ...
        <Link to="/Culture"> <p className='' > Culture</p></Link> 
      </div>
      <h1 className='text-center my-5 heading'>Culture Previous Details</h1>
        <div>
  ...
  {/* <center> <p className='my-2'><b>Total=</b></p></center>   */}
        ...
        <MaterialTable
      title="Culture Previous Details"
      columns={[

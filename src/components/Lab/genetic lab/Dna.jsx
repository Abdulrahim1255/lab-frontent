...
const Dna = (props) => {
    ...
    const [orderBatch,setOrderBatch]=React.useState("G")
      ...
      <Fragment>
       ...
       .then(response=>{
        ...
        if(response){
          ...
          setOrderBatch(`G${response.data[0].count}`)
 }
    ...
    <DialogActions>
      ...
      <Button variant='outlined' color='error'
       onClick={()=>{setShowDialog(false)}}>Cancel</Button>
          ...
          <Link to="/Dnadetails">   <p className='text-right' > previous details</p></Link>  
          </div>

        </Container>
        <h1 className='text-center my-3 mb-5 heading'>DNA</h1>

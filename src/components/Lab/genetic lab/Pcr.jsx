...
const Pcr = (props) => {
    ...
    const [orderBatch,setOrderBatch]=React.useState("A")
...
// console.log(selectedDate)
    ...
    renderCell:()=>(
      <Fragment>
       ...
       const  alldata =()=>{
        ...
        axios.get(`${process.env.REACT_APP_DEVELOPMENT}/api/itemsPrc`,
       ...
       .then(response=>{
        ...
        if(response){
          ...
          setOrderBatch(`A${response.data[0].count}`)
 }
  ...
  const onSubmit= async(data)=>{
    ...
    console.log(obj,'obj')
    try {
       await axios.post(`${process.env.REACT_APP_DEVELOPMENT}/api/addPrc`,obj,
    {headers:{token:`${accessToken}`}})
    .then(response=>{
...
try {
        ...
        console.log(update)
  await  axios.put(`${process.env.REACT_APP_DEVELOPMENT}/api/updatePrc/${update._id}`,combinedObj,
  {headers:{token:`${accessToken}`}})
  .then(response=>{
    console.log('Response',response)
  ...
  const deleteRow = async(update)=>{
        ...
        try {
          ...
          await  axios.delete(`${process.env.REACT_APP_DEVELOPMENT}/api/deletePrc/${update._id}`,update,
    ...
    <DialogActions>
      ...
      <Button variant='outlined' color='error'
       onClick={()=>{setShowDialog(false)}}>Cancel</Button>
          ...
          <Link to="/Pcrdetails">   <p className='text-right' > previous details</p></Link> 
          </div>

        </Container>
        <h1 className='text-center my-3 mb-5 heading'>PCR</h1>
